  ...
  <div class="container">
    ...
    <div class="services-grid">
      ...
      <article class="service-item">
        <div class="icon-wrap">
          @if($service->icon_type === 'svg')
            <svg><use xlink:href="#{{ $service->icon }}"></use></svg>
          @else
            <img src="{{ asset('storage/' . $service->icon) }}" alt="{{ $service->title }}">
          @endif
        </div>
...
<style>
   .ridgeben-services {
  padding: 90px 0;
  background: #f9faff; /* light, clean base */
...
<style>
:root{
  --rbx-teal:#004a87;--rbx-dark:#0f172a;--rbx-muted:#64748b;
  --rbx-surface:rgba(255,255,255,.68);--rbx-brd:rgba(255,255,255,.45);
  --rbx-dur:.8s;--rbx-ease:cubic-bezier(.25,.8,.25,1);
}
#rbx-showcase{position:relative;width:100%;overflow:hidden;border-radius:24px;
  box-shadow:0 10px 30px rgba(0,0,0,.08);font-family:system-ui,sans-serif;
  background:radial-gradient(1200px 600px at 80% -10%,rgba(0,74,135,.08),transparent);
}
.rbx-header{text-align:center;padding-top:8px}
.rbx-pill{display:inline-block;background:rgba(0,74,135,.1);color:var(--rbx-teal);
  padding:10px 24px;border-radius:50px;font-weight:600}
.rbx-title{font-size:2rem;font-weight:800;color:var(--rbx-dark);margin:10px 0 6px}
.rbx-accent{color:var(--rbx-teal)}

.rbx-slides{position:relative;height:560px;isolation:isolate}
.rbx-slide{position:absolute;inset:0;opacity:0;transform:scale(.985);
  transition:opacity var(--rbx-dur) var(--rbx-ease),transform var(--rbx-dur) var(--rbx-ease);
  background-size:cover;background-position:center;display:flex;align-items:center;justify-content:center}
.rbx-slide.active{opacity:1;transform:scale(1)}
.rbx-slide::after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,rgba(0,0,0,.15),rgba(0,0,0,.25));mix-blend:multiply}

.rbx-overlay{position:relative;z-index:2;background:var(--rbx-surface);backdrop-filter:blur(14px);
  border:1px solid var(--rbx-brd);border-radius:18px;padding:36px 44px;max-width:760px;color:var(--rbx-dark);
  box-shadow:0 12px 40px rgba(2,6,23,.12)}
.rbx-topbar{position:absolute;left:0;right:0;top:0;height:3px;background:rgba(2,6,23,.12);overflow:hidden;border-top-left-radius:18px;border-top-right-radius:18px}
.rbx-progress{display:block;height:100%;width:0;background:var(--rbx-teal)}

.rbx-overlay h3{font-size:1.85rem;font-weight:800;margin:4px 0 8px}
.rbx-overlay p{color:var(--rbx-muted);margin:0 0 14px}
.rbx-overlay ul{columns:2;gap:1.25rem;margin:0;padding:0;list-style:disc inside}
.rbx-overlay li{margin:0 0 .55rem}

.rbx-dock{position:absolute;left:50%;bottom:18px;transform:translateX(-50%);
  display:flex;gap:6px;padding:8px;border-radius:999px;background:rgba(255,255,255,.7);
  backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,.5);box-shadow:0 8px 20px rgba(2,6,23,.12)}
.rbx-dock button{appearance:none;border:0;background:transparent;padding:10px 14px;border-radius:999px;
  font-weight:600;font-size:.9rem;color:#0f172a;opacity:.7;cursor:pointer;transition:all .25s}
.rbx-dock button:hover{opacity:1;transform:translateY(-1px)}
.rbx-dock button.active{opacity:1;background:var(--rbx-teal);color:#fff}

@media (max-width: 768px){
  .rbx-slides{height:500px}
  .rbx-overlay{padding:24px;max-width:92%}
  .rbx-overlay ul{columns:1}
  .rbx-dock{bottom:12px}
}
</style>
