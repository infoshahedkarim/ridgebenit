...
@section('body')
    ...
    <div>
        ...
        <form action="{{route('solution.update', $solution->id)}}" method="POST" enctype="multipart/form-data">
            ...
            <div>
                <label>Title</label>
                <input type="text" id="title" placeholder="Title" name="title" value="{{$solution->title}}" required>
            </div>
            <div>
                <label>Short Description</label>
                <input type="text" id="title" placeholder="Short Text" name="subtitle" value="{{$solution->subtitle}}" required>
            </div>
            <div>
                ...
                <select name="service_id" id="service_id" required>
                    ...
                    <option value="{{ $service->id }}"
                        {{ $service->id == $solution->service_id ? 'selected' : '' }}>
                        {{ $service->title }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="row mb-4" style="align-items: center;">
                ...
                <textarea id="detail" name="des" type="text" value="{{$solution->des}}" class="ckeditor form-control"
                    placeholder="Add a details">{{$solution->des}}</textarea>
            </div>
            <div class="row mb-4" style="align-items: center;">
