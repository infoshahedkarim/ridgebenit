...
@endif
        ...
        <form action="{{ route('update.project', $project->slug) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div>
                <label>Title</label>
                <input type="text" id="name" placeholder="Title" name="name" value="{{$project->name}}" onkeyup="generateSlug()" required>
            </div>
            <div class="row mb-4" style="align-items: center;">
                <label for="horizontal-firstname-input" class="col-form-label">Meta Tag</label>
                <input id="slug" name="slug" type="text" class="form-control" value="{{$project->slug}}" placeholder="Add meta tag" />
            </div>
             <div>
                <label>Short Description</label>
                <input type="text" id="title" placeholder="Short Text" name="short_des" value="{{$project->short_des}}" required>
            </div>
             <div>
                <label>Service 1</label>
                <input type="text" id="title" placeholder="Short Text" name="service_1" value="{{$project->service_1}}" required>
            </div>
             <div>
                <label>Service 2</label>
                <input type="text" id="title" placeholder="Short Text" name="service_2" value="{{$project->service_2}}" required>
            </div>
              <div>
            ...
            <div class="row mb-4" style="align-items: center;">
               ...
               <img src="{{ asset('storage/' . $project->img) }}" alt="{{ $project->name }}" width="10%">
            ...
            <div class="row mb-4" style="align-items: center;">
                ...
                <textarea id="detail" name="des" type="text" value="{{$project->des}}" class="ckeditor form-control"
                    placeholder="Add a details">{{$project->des}}</textarea>
            </div>
            <div class="row mb-4" style="align-items: center;">
    ...
    <script>
        ...
        async function checkSlug(baseSlug) {
            ...
            while (exists) {
                const response = await fetch(`/project-slug-check/${slug}`);
